...
<html lang="en">
    ...
    <body>
        ...
        <section class="section-akun">
            ...
            <nav>
                <ul>
                    ...
                    <a href="halaman_hutang.php" class="menu" style="color: #dddddd;">
        ...
        <?php
          $baso="select * from hutang order by id_hutang desc";
          $q=mysqli_query($koneksi, $baso);
          $jum=mysqli_num_rows($q);
        		if($jum > 0){
        			$d=mysqli_fetch_array($q);
        			$urut=substr($d["id_hutang"],4,3)+1;
        				if($urut<10){$id_hutang="HT-00".$urut;}
        				else if($urut<100){$id_hutang="HT-0".$urut;}
        				else{$id_hutang="HT".$urut;}
        			}
        		else{$id_hutang="HT-001";}
        ?>

        <section class="section-main">
            <p id="name-main">Input Transaksi Hutang</p>
            <form class="form-input" action="proses_simpan_hutang.php" method="post">

                <div class="group">
                    <label>Nomor Transaksi</label>
                    <input type="text" name="id_hutang" required="required" style="width: 200px; height: 26px; border-radius: 3px; background-color: #ffffff; font-size:0.8em; padding-left:10px;" value="<?php echo $id_hutang;?>">
                </div>
                <div class="group">
                    <label>Tanggal</label>
                    <input class="input2" type="date" name="tgl_transaksi_htng" required="required" style="width: 200px; height: 26px; border-radius: 3px; background-color: #ffffff; padding-left:10px;">
                </div>

                <div class="group2">
                    <label>Jumlah</label>
                    <input type="number" name="jumlah_hutang" placeholder="jumlah=400000"required="required" style="width: 300px; height: 26px; border-radius: 3px; background-color: #ffffff; font-size:0.8em; padding-left:10px;">
                </div>
                <div class="group2">
